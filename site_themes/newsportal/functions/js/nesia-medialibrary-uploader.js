/*-----------------------------------------------------------------------------------*/
/* Nesia Media Library-driven AJAX File Uploader Module - JavaScript Functions */
/* 2010-11-05. */
/*
/* The code below is designed to work as a part of the Nesia Media Library-driven
...
/*-----------------------------------------------------------------------------------*/
	...
	$( 'input.upload_button' ).removeAttr( 'style' );
      ...
      $( 'input.upload_button').live( "click", function () {
       ...
       if ( $(this).parents( '.section' ).find( '.heading' ) ) { nesia_title = $(this).parents( '.section' ).find( '.heading' ).text(); } // End IF Statement
       
       if ( nesia_title == '' && $(this).parents( '.nesia_metabox_fields' ).prev( 'th' ).find( 'label' ) ) { nesia_title = $(this).parents( '.nesia_metabox_fields' ).prev( 'th' ).find( 'label' ).text(); } // End IF Statement
        
        tb_show( nesia_title, 'media-upload.php?post_id='+formID+'&amp;title=' + nesia_title + '&amp;is_nesia=yes&amp;TB_iframe=1' );
        return false;
      });
  ...
  }; // End nesiaMLU Object // Don't remove this, or the sky will fall on your head.

/*-----------------------------------------------------------------------------------*/
/* Execute the above methods in the nesiaMLU object.
/*-----------------------------------------------------------------------------------*/
  
	$(document).ready(function () {

		nesiaMLU.removeFile();
		nesiaMLU.recreateFileField();
		nesiaMLU.mediaUpload();
